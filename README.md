...
## Clickables
"Clickables" are things you can put in the portrait panel that cause a scene to be played when they're clicked. They can be shown and removed, with or without transitions, just like characters.
...
| `g.removeCharacter(characterName, options)` | Removes a specified character's portrait from the screen. | transition: `fade` to fade out, `slide` to slide out. <br /><br /> direction: The direction the portrait will slide out towards, if `transition: 'slide'` is given. (`up`, `down`, `left`, or `right`.) <br /><br /> speed: The time (in seconds) that a specified transition should take. | |
| `g.showClickable(targetScene, imagePath, options)` | Adds a clickable image to the scene. | transition: `fade` to fade in, `slide` to slide in. <br /><br /> direction: The direction the portrait will slide in from, if `transition: 'slide'` is given. (`up`, `down`, `left`, or `right`.) <br /><br /> speed: The time (in seconds) that a specified transition should take. | |
| `g.removeClickable(targetScene, options)` | Removes a specified clickable portrait from the screen. | transition: `fade` to fade out, `slide` to slide out. <br /><br /> direction: The direction the portrait will slide out towards, if `transition: 'slide'` is given. (`up`, `down`, `left`, or `right`.) <br /><br /> speed: The time (in seconds) that a specified transition should take. | |
| `g.removeAll(options)` | Removes all clickables and character portraits from the screen at once. | transition: `fade` to fade out, `slide` to slide out. <br /><br /> direction: The direction the portraits will slide out towards, if `transition: 'slide'` is given. (`up`, `down`, `left`, or `right`.) <br /><br /> speed: The time (in seconds) that a specified transition should take. | |
